...
def introduccion_with():
    ...
    print("\n¿Qué es `with`?")
    print(
        "- Es un gestor de contexto que facilita la gestión de recursos, como archivos."
    )
    print(
        "- Asegura que los archivos se cierren automáticamente al finalizar el bloque."
    )
...
def actividad_interactiva():
    ...
    nombre_archivo = input(
        "Ingresa el nombre del archivo que deseas crear o leer (con extensión): "
    )

    # Escribir en el archivo
    print(
        "Escribe las líneas que deseas agregar al archivo. Escribe 'fin' para terminar:"
    )
    with open(nombre_archivo, "w") as archivo:
        while True:
            linea = input("Línea: ")
    ...
    if opcion == "sí":
        ...
        print(
            "Actividad completada. Puedes abrir el archivo manualmente para verificar el contenido."
        )
